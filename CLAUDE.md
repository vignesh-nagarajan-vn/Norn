...
- The running-state `PipelineView` shows a bronze loading bar (`loom-shimmer` sheen) that fills from the streamed stage events; the in-flight stage gets half credit so it advances mid-stage.
- `components/Structure3D.tsx` renders a 3D view of the variant: it maps the gene to a UniProt accession (a hardcoded map, extend as needed), fetches the AlphaFold model, and renders it with 3Dmol.js (lazy-loaded from a jsDelivr CDN), highlighting the residue in `--secondary`. It renders `null` when the gene is unmapped or the protein position is unknown, and a graceful message if the structure or viewer is unreachable. AlphaFold and the CDN are fetched from the browser, so it works on the live deploy even though this sandbox's network blocks `alphafold.ebi.ac.uk` (returns a proxy 404/403). Colors are read from CSS vars so it follows the theme.
