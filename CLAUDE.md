...
- Every fetch has a timeout and graceful degradation (a failed source marks its criteria unknown). Most sources retry once; `variant_recoder` fails fast (one attempt, 6s timeout) because it is non-critical (VEP supplies the gene, consequence, and coordinates), so a slow normalizer no longer stalls the whole run at ~16s.
...
- The running-state `PipelineView` shows a bronze loading bar (`loom-shimmer` sheen) in its own `ThreadBar` subcomponent. The real stages are lopsided (recode + VEP run first, in parallel, and take almost all the time; gnomAD, ClinVar, and the two Claude passes finish in a rush), so a stage-event-driven bar sat near 0 then jumped straight to done. Instead a `requestAnimationFrame` loop creeps the bar toward 100% on a time constant (`timeTarget = 100*(1 - exp(-elapsed/TAU))`, smooth and even, never stalling), floored by the real fraction of stages done so it never runs ahead of reality, eased per frame (`prev + (target - prev) * (1 - exp(-k*dt))`, monotonic) and snapped to 100% when the run finishes. A rough `~Ns left` estimate (a fixed `EST_TOTAL` minus elapsed, floored at ~2s) counts down beside the percentage; an exact ETA is impossible with variable public-API latency, so it is shown as an estimate. The bar `width` is driven by that float (no CSS width transition), the component is isolated so per-frame updates never re-render the stage nodes, and it snaps without animation under `prefers-reduced-motion`.
