...
- The running-state `PipelineView` shows a bronze loading bar (`loom-shimmer` sheen) driven by an eased trickle (an interval that catches up to the completed-stage floor and creeps toward the next stage while one is in flight), so it climbs smoothly instead of jumping between stage events.
