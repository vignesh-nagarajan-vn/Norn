...
/*
  ...
  replayed or skipped. Opened from the landing (button + first visit + ?demo=1).
*/
        ...
        return (
          <div key={s} className="flex flex-1 items-center last:flex-none">
            <div className="flex flex-col items-center gap-1.5">
              ...
              <span className="mono text-[11px]" style={{ color: on ? "var(--on-surface)" : "var(--on-surface-variant)" }}>{s}</span>
            </div>
            {i < STAGES.length - 1 && (
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-baseline justify-between">
        ...
        <span className="mono text-3xl font-semibold text-lpath">+9 pts</span>
      </div>
      <div className="relative h-6 overflow-hidden rounded border border-outline-variant bg-surface-high">
...
function Stage({ phase }: { phase: number }) {
  return (
    <div className="card min-h-[340px] p-6 shadow-thread">
      <div className="mb-5 flex items-center justify-between">
        <span className="mono text-base font-bold text-on-surface">{VARIANT}</span>
      ...
      <div className="mt-6">
        {phase === 0 && (
          <div className="flex flex-col items-center gap-3 py-8 text-center">
            <NornMark size={48} className="thread-draw text-secondary" strokeWidth={2.2} />
            <div className="mono text-xl text-on-surface">
              {VARIANT}
              <span className="animate-norn-pulse">|</span>
            </div>
            <p className="text-[15px] text-on-surface-variant">Reading one variant, end to end.</p>
          </div>
        )}
        ...
        {phase === 1 && (
          <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
            {GATHER.map((g, i) => (
              <div key={g.label} className="fate-rise flex items-start gap-2.5 rounded-md border border-outline-variant bg-surface-bright p-3" style={{ animationDelay: `${i * 0.5}s` }}>
                <Icon name={g.icon} size={20} className="mt-0.5 text-secondary" />
                <div>
                  <div className="label-caps">{g.label}</div>
                  <div className="text-sm text-on-surface">{g.fact}</div>
                </div>
              </div>
            ))}
          </div>
            ...
            {CRITERIA.map((c, i) => (
              <div key={c.code} className="fate-rise relative flex border-b border-outline-variant/60 bg-surface-bright pl-3 last:border-0" style={{ animationDelay: `${i * 0.6}s` }}>
                ...
                <div className="flex flex-1 items-center justify-between gap-3 p-3">
                  ...
                  <span className="inline-flex items-center rounded px-2 py-0.5 text-[10px] font-bold" style={c.met ? { background: "color-mix(in srgb, var(--pathogenic) 16%, var(--surface-bright))", color: "var(--pathogenic)" } : { background: "var(--surface-variant)", color: "var(--on-surface-variant)" }}>{c.met ? "MET" : "NOT MET"}</span>
                  <p className="flex-1 text-[13px] text-on-surface-variant">{c.ev}</p>
          ...
          <div className="space-y-5">
            ...
            {phase === 4 && (
              <div className="flex flex-wrap items-center gap-3 border-t border-outline-variant pt-5">
                ...
                <span className="text-sm text-on-surface-variant">Run your own variant.</span>
              </div>
            )}
          </div>
        )}
...
export default function GuidedDemo({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col bg-background" role="dialog" aria-modal="true" aria-label="Norn guided tour">
      {/* header */}
      <div className="flex items-center justify-between border-b border-outline-variant bg-surface px-6 py-3">
        <div className="flex items-center gap-2.5">
          <NornMark size={24} className="text-secondary" />
          <span className="display text-xl font-semibold text-on-surface">Norn guided tour</span>
        </div>
      ...
      {/* per-phase progress */}
      <div className="h-1 w-full bg-outline-variant/50">
        {phase < 4 && (
      ...
      <div className="flex flex-1 items-center justify-center overflow-y-auto px-6 py-10">
        <div className="grid w-full max-w-6xl grid-cols-1 gap-10 lg:grid-cols-[1.5fr_1fr] lg:items-center">
          <Stage phase={phase} />

          <div>
            <div className="eyebrow mb-3">
              <NornMark size={15} className="text-secondary" strokeWidth={2.4} />
              {phase === 0 ? "The thread" : phase === 4 ? "Decreed" : `Step ${phase} of 3`}
            </div>
            {fate.norse ? (
              <div className="mb-1 flex items-baseline gap-2.5">
                <span className="display text-4xl font-semibold text-on-surface">{fate.norse}</span>
                <span className="text-base italic text-on-surface-variant">{fate.gloss}</span>
              </div>
            ) : null}
            <h2 className="display text-3xl font-semibold tracking-tight text-on-surface">{fate.title}</h2>
            <p className="mt-4 text-base leading-relaxed text-on-surface-variant">{fate.body}</p>
      ...
      <div className="border-t border-outline-variant bg-surface px-6 py-2.5 text-center text-xs text-on-surface-variant">
